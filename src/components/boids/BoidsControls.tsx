...
const PARAMS = [
  { key: 'matchingFactor', label: 'Alignment', min: 0, max: 0.1, step: 0.001, default: 0.016 },
  { key: 'centeringFactor', label: 'Cohesion', min: 0, max: 0.001, step: 0.00001, default: 0.0005 },
  { key: 'avoidFactor', label: 'Separation', min: 0, max: 0.02, step: 0.0001, default: 0.015 },
  { key: 'turnFactor', label: 'Turn Factor', min: 0, max: 0.5, step: 0.01, default: 0.14 },
  { key: 'maxSpeed', label: 'Max Speed', min: 1, max: 10, step: 0.1, default: 6 },
  { key: 'minSpeed', label: 'Min Speed', min: 0.5, max: 5, step: 0.1, default: 2.5 },
  { key: 'visionRadius', label: 'Vision Radius', min: 10, max: 300, step: 1, default: 120 },
  { key: 'avoidRadius', label: 'Avoid Radius', min: 5, max: 100, step: 1, default: 25 },
  { key: 'numFlocks', label: 'Number of Flocks', min: 1, max: 6, step: 1, default: 1 },
]

export default function BoidsControls({ onClose }: { onClose: () => void }) {
  const { getParamsRef, updateParamsRef, setFlockCountRef } = useBoids()
  ...
  const handleChange = (key: string, value: number) => {
    ...
    setValues(next)
    if (key === 'numFlocks') {
      setFlockCountRef.current?.(value)
    } else {
      ...
      >
        ...
        <div className="mt-6 flex justify-between">
          ...
          <button
            className="text-sm text-gray-500 hover:text-white transition-colors"
            onClick={() => {
              const random = Object.fromEntries(PARAMS.filter(p => p.key !== 'numFlocks').map(p => [
                p.key,
